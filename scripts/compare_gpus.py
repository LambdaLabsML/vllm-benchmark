...
def plot_benchmark_data(num_prompts, gpu_names, base_task_name, result_path, render_path):
    ...
    for idx, gpu_name in enumerate(gpu_names):
        ...
        plt.plot(list_median_itl_ms, list_output_throughput, marker='o', color=colors(idx), label=gpu_name)
    
    # Modify the base_task_name to remove the "_tp" part for the title
    title_task_name = base_task_name.replace("_tp", "").replace(f"_tp{tp_value}", "")
...
gpu_names = ["1xA100-80GB-SXM", "2xA100-80GB-SXM"]  # Replace with your actual GPU names
